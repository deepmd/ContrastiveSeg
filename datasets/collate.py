...
def stack(batch, data_key=None, return_dc=False):
    ...
    #
    # else:
    result = default_collate([sample[data_key] for sample in batch])
    return result
...
def collate(batch, trans_dict):
    ...
    for i in range(len(batch)):
        ...
        # Log.info('{} {} {} {} {}'.format(target_width, target_height, scaled_size[0], scaled_size[1],
        #                                  trans_dict['align_method']))
        ...
        if pad_width > 0 or pad_height > 0:
            ...
            batch[i]['img'] = F.pad(batch[i]['img'].data, pad=pad, value=0)

            if 'labelmap' in data_keys:
                batch[i]['labelmap'] = F.pad(batch[i]['labelmap'].data, pad=pad, value=-1)

            if 'maskmap' in data_keys:
                batch[i]['maskmap'] = F.pad(batch[i]['maskmap'].data, pad=pad, value=0)

            if 'distance_map' in data_keys:
                batch[i]['distance_map'] = F.pad(batch[i]['distance_map'].data, pad=pad, value=255)

            if 'angle_map' in data_keys:
                batch[i]['angle_map'] = F.pad(batch[i]['angle_map'].data, pad=pad, value=0)
 
            if 'mask_label_map' in data_keys:
                batch[i]['mask_label_map'] = F.pad(batch[i]['mask_label_map'].data, pad=pad, value=-1)

            if 'direction_label_map' in data_keys:
                batch[i]['direction_label_map'] = F.pad(batch[i]['direction_label_map'].data, pad=pad, value=-1)

            if 'multi_label_direction_map' in data_keys:
                batch[i]['multi_label_direction_map'] = F.pad(batch[i]['multi_label_direction_map'].data, pad=pad, value=-1)
                
            if 'energy_label_map' in data_keys:
                batch[i]['energy_label_map'] = F.pad(batch[i]['energy_label_map'].data, pad=pad, value=-1)

            if 'offsetmap_h' in data_keys:
                batch[i]['offsetmap_h'] = F.pad(batch[i]['offsetmap_h'].data, pad=pad, value=0)

            if 'offsetmap_w' in data_keys:
                batch[i]['offsetmap_w'] = F.pad(batch[i]['offsetmap_w'].data, pad=pad, value=0)
