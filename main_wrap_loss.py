...
from networks.hrnet import HRNet_W48
from optim_scheduler import get_optim_scheduler
from losses import FSCELoss
...
def parse_options():
    ...
    parser.add_argument('--model', type=str, default='HRNet-W48', choices=['HRNet-W48'])
...
def set_model(opt):
    ...
    model = None
    if opt.model == 'HRNet-W48':
        model = HRNet_W48(num_classes=opt.num_classes, bn_type='torchbn')
    else:
    ...
    criterion = FSCELoss(opt.ce_weights, opt.ce_ignore_index)
...
def train_validate(model, criterion, data_loaders, optimizer, scheduler, opt):
    ...
    for data_dict in data_loaders['train']:
        ...
        opt.tb_logger.add_scalar('train/ce_loss', loss, opt.current_iter)
...
def validate(model, val_loader, criterion, opt):
    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(tqdm(val_loader)):
            ...
            loss = criterion(logits, targets)
            loss = loss.mean() if torch.cuda.device_count() > 1 else loss
            # logits = F.interpolate(logits, [targets.size(1), targets.size(2)],
...
def main():
    ...
    while opt.current_iter < opt.max_iters:
        ...
        time1 = time.time()
        loss = train_validate(model, criterion, data_loaders, optimizer, scheduler, opt)
        time2 = time.time()
        logger.info(f"[End Epoch {opt.current_epoch-1}] Train Time: {(time2 - time1):0.2f}, Loss: {loss:06.3f}")
        opt.tb_logger.add_scalar('train/ce_loss_avg', loss, opt.current_epoch)

    # # last evaluation
    # class_ious, pixel_acc = validate(model, val_loader)
    # logger.info(f"[Validation] [Epoch {opt.current_epoch}] " +
    #             f"Mean IOU: {torch.mean(class_ious).item():.3f}" +
    #             f"Pixel ACC: {pixel_acc} " +
    #             f"Class IOU: <{'.'.join(f'{idx}: {iou}' for idx, iou in enumerate(class_ious))}>")
    # opt.tb_logger.add_scalar('val/mIOU', torch.mean(class_ious), opt.current_iter)
    # opt.tb_logger.add_scalar('val/pixel_acc', pixel_acc, opt.current_iter)
    #
    # # save the last model
    # save_file = os.path.join(opt.save_folder, 'last.pth')
    # save_checkpoint(model, optimizer, opt, opt.max_iters, save_file)
