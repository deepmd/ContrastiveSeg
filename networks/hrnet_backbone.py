...
from .modules import BatchNorm2d
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, bn_type=None, bn_momentum=0.1):
        ...
        self.bn1 = BatchNorm2d(bn_type=bn_type)(planes, momentum=bn_momentum)
        self.relu = nn.ReLU(inplace=False)
        self.relu_in = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = BatchNorm2d(bn_type=bn_type)(planes, momentum=bn_momentum)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, bn_type=None, bn_momentum=0.1):
        ...
        self.bn3 = BatchNorm2d(bn_type=bn_type)(planes * 4, momentum=bn_momentum)
        self.relu = nn.ReLU(inplace=False)
        self.relu_in = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            BatchNorm2d(bn_type=bn_type)(num_inchannels[i], momentum=bn_momentum),
                        )
                    )
                elif j == i:
                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    ...
                                    BatchNorm2d(bn_type=bn_type)(num_outchannels_conv3x3, momentum=bn_momentum)
                                )
                            )
                        else:
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, bn_type, bn_momentum, **kwargs):
        ...
        if os.environ.get('full_res_stem'):
            ...
            self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=stem_stride, padding=1,
                                   bias=False)
            self.bn1 = BatchNorm2d(bn_type=bn_type)(64, momentum=bn_momentum)
            self.relu = nn.ReLU(inplace=False)
            ...
            self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=stem_stride, padding=1,
                                   bias=False)
            self.bn2 = BatchNorm2d(bn_type=bn_type)(64, momentum=bn_momentum)
            self.relu = nn.ReLU(inplace=False)
            self.layer1 = self._make_layer(Bottleneck, 64, 64, 4, bn_type=bn_type, bn_momentum=bn_momentum)
    ...
    def _make_head(self, pre_stage_channels, bn_type, bn_momentum):
        ...
        for i in range(len(pre_stage_channels) - 1):
            ...
            downsamp_module = nn.Sequential(
                nn.Conv2d(in_channels=in_channels,
                          ...
                          stride=2,
                          padding=1),
                BatchNorm2d(bn_type=bn_type)(out_channels, momentum=bn_momentum),
                nn.ReLU(inplace=False)
            )
        ...
        final_layer = nn.Sequential(
            ...
            BatchNorm2d(bn_type=bn_type)(2048, momentum=bn_momentum),
            nn.ReLU(inplace=False)
        )
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            ...
                            BatchNorm2d(bn_type=bn_type)(num_channels_cur_layer[i], momentum=bn_momentum),
                            nn.ReLU(inplace=False)
                        )
                    )
                else:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            ...
                            BatchNorm2d(bn_type=bn_type)(outchannels, momentum=bn_momentum),
                            nn.ReLU(inplace=False)
                        )
                    )
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, bn_type=None, bn_momentum=0.1):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                BatchNorm2d(bn_type=bn_type)(planes * block.expansion, momentum=bn_momentum)
            )
